window.onload = function() {
	...
	function _setMyCompanyInfo(res){
		let _obj = res.data.myCompanyInfo;
	...
	$("#myModPwdProcBtn").on("click", function(){
		...
		let _p1 = $("#myModPwd1").val();
		let _p2 = $("#myModPwd2").val();
	...
	function _setMyMemberList(res){
		let _list = res.data.myMemberList;
		let _target = $("#myMemberListUl");
		...
		if(_list.length == 0){
			...
			for(let i=0; i<_list.length; i++){
				...
				let _memberCreateDateDt = _list[i].memberCreateDateDt;
		...
		//	삭제 이벤트
		let _delBtn = $("#myMemberListUl").find(".close");
		$(_delBtn).on("click", function(e){
			
			if(e.target.className == 'close'){
				let _memberId = $(this).parent('.myMemberListLi').attr('data-member-id');
				let _memberIdx =  $(this).parent('.myMemberListLi').attr('data-member-idx')
	...
	$("#modifyModalBtn").on("click", function(){
		let _active = $("#myMemberListUl").find(".active")
		if(_active.length == 0){
