window.onload = function() {
	...
	let api = {
		"sensorSelect" : "/manager/sensor/select",
		"sensorLogList" : "/manager/sensor/logList",
		"sensorLogChart" : "/manager/sensor/searchChartLog"
	}
	...
	function select(index) {
		...
		options.forEach((item) => {
			item.addEventListener('click', function(e) {
				...
				if(selectInput.getAttribute("id")=="companyInput"){
					let _companyIdx = $(this).attr("data-company-idx");
					$(selectInput).attr('data-company-idx', _companyIdx);
					
					let _obj = {
						"companyIdx" : _companyIdx
					}
					
					_doAjaxGet(api.sensorSelect, _obj, _setSensorSelect);
	...
	function _setSensorSelect(res){
		//console.log(res);
		let _list = res.data.sensorSelectList;
		let _target = $("#sensorSelect");
		$(_target).find("li").remove();

		if(_list.length == 0){
			$("#sensorInput").attr("data-sensor-idx", 0);
			$("#sensorInput").val('전체선택')
		}else{
			for(let i=0; i<_list.length; i++){
				let _sensorIdx = _list[i].sensorIdx;
				let _sensorLocationDescription = _list[i].sensorLocationDescription;
				_li = `
		...
		if(_targetLi.length !=0){
			let _liFirst = _targetLi[0];
			let _sensorIdx = $(_liFirst).attr('data-sensor-idx')
			let _text = $(_liFirst).text();
			$("#sensorInput").attr("data-sensor-idx", _sensorIdx);
			$("#sensorInput").val(_text)
		}
	}
	...
	$("#searchBtn").on("click", function(){
		_sensorLogObjInit();
		_sensoChartLogInit();
		let _sensorIdx = $("#sensorInput").attr("data-sensor-idx"); 
	...
	function _setSensorLogList(res){
		...
		let _list = res.data.sensorLogList;
		
		let _target = $("#logTable");
		...
		if(_list.length == 0){
			...
			for(let i=0; i<_list.length; i++){
				let _companyName = _list[i].companyName;
				let _sensorId = _list[i].sensorId;
				let _sensorTypeName = _list[i].sensorTypeName;
				let _sensorTypeCode = _list[i].sensorTypeCode;
				let _sensorLocationDescription = _list[i].sensorLocationDescription;
				let _sensorLogCreateDate = _list[i].sensorLogCreateDate;
				let _sensorLogValue = koreasoft.modules.string.getValueType(_sensorTypeCode ,_list[i].sensorLogValue);
				let _sensorApprovalDate = _list[i].sensorApprovalDate;
				let _tr = `
					<tr class='sensorLog'>
						<td>${_companyName}</td>
						<td>${_sensorId}</td>
	...
	function _makePageEvt(page){
		...
		let _target = $("#sensorLogPageUl")
		let _liClassName = 'sensorLogPageLi';
		$(_target).find("li").remove();
	...
	$("#csvDown").on("click", function(e){
		let _target = $("#logTable").find(".noData")
		if(_target.length > 0 ){
	...
	$("#csvProcBtn").on("click", function(){
		...
		let _url = `/api/v1/sensor/downloadCSV?sensorIdx=${_obj.sensorIdx}&startDate=${_obj.startDate}&endDate=${_obj.endDate}&fileName=${_obj.fileName}`;
