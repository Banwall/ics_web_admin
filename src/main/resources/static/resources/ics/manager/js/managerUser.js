window.onload = function() {
	...
	var api = {
		...
		"companyInfo" : "/manager/companyInfo",
	...
	function _makeCompanyLiEvt(){
		...
		$(_li).on("click", function(){
			...
			var _stat = $(this).attr('data-company-status')
			if(_stat == 'Y'){
	...
	function _setCompanyInfo(res){
		...
		var _data = companyRole[_obj.companyRole];
	...
	function _pwdCheck(type){
		var _p1 = type == 1 ? $("#addCompanyPwd1").val() : $("#modPwd1").val();
		var _p2 = type == 1 ? $("#addCompanyPwd2").val() : $("#modPwd2").val();
		var _target = type == 1 ? $("#addCompanyPwdCheck") : $("#modPwdError");
		var _text = null;
		//console.log(type)
		if(_p1 != _p2){
	...
	function _addCompanyMemberIsIdCallback(res){
		...
		var _text = null;
		var _target = $("#addCompanyMemberError");
		$(_target).text("");
	...
	var _popCompanyName = null;
	function _popModal(type, v){
		_modalType = type;
		
		var _headerTarget = $("#agreeHeader");
		var _textTarget = $("#agreeText");
		var _title = null;
		var _text = null;
	...
	$("#agreeModalBtn").on("click", function(){
		if(_modalType == 1){
			...
			//	회원 등록
			var _p1 = $("#addCompanyPwd1").val();
			var _p2 = $("#addCompanyPwd2").val();
			...
			//	승인
			var _li = $("#memberListLi").find("li.active");
			var _obj = {
			...
			var _roleType = $("#memberInfoInput").attr('data-role-search-type');
			
			var _role = Number(_roleType) == 2 ? 'ROLE_O' : 'ROLE_M';
