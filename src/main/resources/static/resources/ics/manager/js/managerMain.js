window.onload = function(){
	...
	function makeMap() {
		...
		function writeIotBox(floor) {
			...
			rStationSensor[floor].data.forEach((item, index) => {
				ImgContainer.innerHTML += makeRStationIcon(`rStation${index}`, item)
			})
		...
		function makeRStationIcon(index, position) {
			return `<div class="rStation-icon ${index}" data-sensor-id='${position.sensorId}' data-type='3' style="left:${position.left}; top:${position.top}; display:${position.display}">
                                <span>
                                </span>
                            </div>`
		}
